...
(function( window, undefined ) {
	...
	$.Flips.prototype 	= {
		...
		_loadEvents			: function() {
			...
			this.$flipPages.find( '.box' ).on( 'tapone.flips', function( event ) {
				console.log(this);
				var lat = $(this).attr('data-lat'), 
				    lng = $(this).attr('data-lng'), 
					heading = parseInt($(this).attr('data-heading')),
					pitch = parseInt($(this).attr('data-pitch')),
					sound = 'sound/' + $(this).attr('data-sound');
				...
				var $box 			= $(this),
					...
					.html('<div id="map-canvas" class="streetview"></div><audio src="'+sound+'" autoplay loop></audio>')
					.css( {
						'position' 			: 'absolute',
						'width'				: prop.width,
						'height'			: prop.height,
					...
					.insertAfter( $overlay )
					.end()
					.append( $boxClose.on( 'tapone.flips', function( event ) {
					...
					.on( 'webkitTransitionEnd.flips transitionend.flips OTransitionEnd.flips', function( event ) {
						...
						initialize(lat, lng, heading, pitch);
